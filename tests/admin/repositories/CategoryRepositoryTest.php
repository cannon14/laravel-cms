...
class CategoryRepositoryTest extends TestCase
{
    ...
    public function testCreateObject()
    {
		...
		$this->assertInstanceOf('cccomus\Models\Category', $object);
    }
	...
	public function testUpdateOrCreate() {
		...
		$object = $this->repo->getObject(8);

		$this->assertEquals('Test Category', $object->name);
		$this->assertEquals('description', $object->description);
		$this->assertEquals('test-category', $object->slug);
		$this->assertEquals(1, $object->active);

		//Run this so we get a failure if we add or subtract database columns
		$this->assertEquals(9, count($object->getAttributes()));
	...
	public function testGetCategoriesList() {
		$list = $this->repo->getCategoriesList();

		//Seeder added 7 categories
