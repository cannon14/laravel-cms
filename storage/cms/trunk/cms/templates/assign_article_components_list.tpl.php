...
function changeArticle(){
	...
	document.location = "index.php?mod=<?=$_REQUEST['mod']?>&siteId=<?=$_REQUEST['siteId']?>&articlecategoryid=<?=$_REQUEST['articlecategoryid']?>&articleid="+select.value;
}
</script>
</head>
<center>
<select id='articleSelect' onChange=javascript:changeArticle();>
	<option>--Choose an Article--</option>
	<?foreach($this->articles as $article){
		if($_REQUEST['articleid'] == $article['id'])$selected="selected";
		else $selected='';
		echo "<option value=".$article['id']." $selected>".$article['title']."</option>";
	}?>
</select>
<form name='editComponent' method=POST>
<input type="checkbox" value='1' name="applyAll">Apply layout to all articles<br>
...
<input type='hidden' name='artcileid' value='<?=$_REQUEST['articleid']?>'>
<input type='hidden' name='siteId' value='<?=$_REQUEST['siteId']?>'>
<input type='button' value='Done' onClick="window.location='index.php?mod=CMS_view_siteToArticleCategory&siteId=<?=$_REQUEST['siteId']?>'">
</center>
