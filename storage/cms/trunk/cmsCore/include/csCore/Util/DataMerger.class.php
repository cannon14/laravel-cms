 ...
 */
 class csCore_Util_DataMerger 
 {
 	...
 	function csCore_Util_DataMerger($e, $ma, $table, $key = null)
 	{
 		$this->regEx = "(".$e.").+(".$e.")";
 		$this->delim = $e;
 		if(is_array($ma)){
 	...
 	function _mapSql($table, $key)
 	{
 		...
 		while($rs && !$rs->EOF){
	 		foreach($this->currentMapping as $key=>$val){ 			
	 			if(is_array($val)){
	 				if(!isset($val[$rs->fields[$key]])) $val[$rs->fields[$key]] = '';
	 				$this->internalMapping[$rs->fields[$this->priKey]][$key] = $val[$rs->fields[$key]];
	 			}else{
		 			if($rs->fields[$val] == 0) $rs->fields[$val] = 0;
