...
*/
class csCore_UI_SLLists {

	var $lists = array();
	var $jsPath;
	var $debug = false;
	
	function csCore_UI_SLLists($jsPath) {
		$this->jsPath = $jsPath;
	}
	...
	function printTopJS() {
		...
		<script src="<?=$this->jsPath;?>/util.js" type="text/javascript"></script>
		<script src="<?=$this->jsPath;?>/effects.js" type="text/javascript"></script>
		<script src="<?=$this->jsPath;?>/dragdrop.js" type="text/javascript"></script>
		<script src="<?=$this->jsPath;?>/controls.js" type="text/javascript"></script>
	...
	function printForm($action, $method = 'POST', $submitText = 'Submit', $md = '', $idName, $submitClass = '',  $formName = 'sortableListForm') {
		?>
		<form action="<?=$action;?>" method="<?=$method;?>" onSubmit="populateHiddenVars();" name="<?=$formName;?>" id="<?=$formName;?>">
			...
			<input type="hidden" name="mod" value="<?=$md?>">
			<input type="hidden" name=<?=$idName?> value="<?=$_REQUEST[$idName]?>">
			<?
			if ($this->debug) {
				?>
