...
class CMS_libs_SiteCatalystIndividualCards
{
	/**
    ...
    * @param int fid
	...
	function savePageVarData($detailId, $pageVarData)
	{
      ...
      {
         // delete
         $sql = 'delete from sc_individual_card_page_data where card_details_id = ' . _q($detailId);
         // echo $sql.'<br>';
         $rs = _sqlQuery($sql, __LINE__, __FILE__, DEBUG_MODE);
         
         // re insert
         $sql = 'insert into sc_individual_card_page_data (card_details_id, var_name, var_value) values';
   ...
   function getPageVarValues($detailId)
   {
      ...
      select          
          ...
          d.var_value
      from sc_page_vars v
      left outer join sc_individual_card_page_data d on v.var_name = d.var_name
         and d.card_details_id = '._q($detailId);
