...
var s_account="ccardsccdc-us,ccardsdevelopment"
var s=s_gi(s_account)

/************************** CONFIG SECTION **************************/
/* You may add or alter any code config here. */
s.trackingServer="cctrkom.creditcards.com"
s.trackingServerSecure="scctrkom.creditcards.com"
/* Conversion Config */
s.currencyCode="USD"
...
s.linkInternalFilters="javascript:,creditcards.com"
s.linkLeaveQueryString=false
s.linkTrackVars=""
s.linkTrackEvents=""
s.gmt='-6';  //hour difference from GMT for time parting set for partner time zone
...
function s_doPlugins(s){
	...
	/* When the purchase event is set, set the transactionID and transactionID eVar */
	if(s.events){
		if(s.events.indexOf("purchase")>-1){
			if(s.purchaseID){
				s.eVar13=s.transactionID=s.purchaseID;
			}
			s.temppid=s.getQueryParam('pid');
			if(s.temppid && s.eVar25 && s.eVar26){
				s.eVar29=s.eVar25+":"+s.temppid+"|"+s.eVar26;
				s.eVar30=s.eVar25+":"+s.temppid;
				s.eVar31=s.temppid+"|"+s.eVar26;
				s.eVar32=s.eVar25+"|"+s.eVar26;
			}
		}
	}
	...
	/* Encode SearchCenter Tracking Code */
	if(s.getQueryParam('s_kwcid')){
		s.pageURL=s.manageQueryParam('s_kwcid',1,1);
	}

	/* SearchCenter Bounce Rate */
	s.clickThruQuality('s_kwcid','event17','event18');

	/* Timeparting */
	s.eVar15=s.prop9=s.getTimeParting('h',s.gmt,new Date().getFullYear());  // Set hour
	s.eVar16=s.prop10=s.getTimeParting('d',s.gmt,new Date().getFullYear()); // Set day
	s.eVar17=s.prop11=s.getTimeParting('w',s.gmt,new Date().getFullYear()); // Set Weekend or Weekday

	/* Campaign Stacking */
	s.eVar14=s.crossVisitParticipation(s.campaign,'s_cpm','90','5','>','event2',0);

	/* Copy PageName into eVar*/
	if(s.pageName){
		s.eVar28=s.pageName;
	}
...
/*
 * Plugin: getTimeParting - Set timeparting values based on time zone
 */
...
s.visitorNamespace="creditcardscom"
s.dc=112
