...
{
  echo "<a href=index_popup.php?md=QUnit_Help_Help&hid=$helpID target=helpwnd><img src='./images/help.gif' border=0></a>";
}
...
<tr>
  <td class=help align=left valign=top>
...
function email_send($recipient, $subject, $text, $messagetouserid = '')
{
    if($recipient == '')
    {
        $errorMsg = "Email sending failed, because the recipient email address was empty. Subject of email: '$subject'";
    ...
    if($subject == '' && $text == '')
    {
        $errorMsg = "Email sending failed, because both subject and text of email were empty. Email recipient: '$recipient'";
        LogError($errorMsg, __FILE__, __LINE__);
        return true;
    }
...
/*  //uncomment this if you want to support special characters in emails
    ...
    $header = "From: ".$GLOBALS['Auth']->getSetting('system_email')."\r\n". 
              "MIME-Version: 1.0\r\n". 
...
*/
    ...
    $sent = @mail($recipient, $subject, $text, "From: ".$GLOBALS['Auth']->getSetting('system_email')."\r\n"); 

    if($sent && $messagetouserid != '')
    {
        $sql = 'update wd_g_messagestousers set rstatus='._q(MESSAGESTATUS_SHOW).
               ' where messagetouserid='._q($messagetouserid);
...
{
    QCore_History::writeHistory(HISTORY_ERROR, $log_msg, $file, $line);
}
...
function LogMsg($log_msg, $file = '', $line = '')
{
    ...
    QCore_History::writeHistory(HISTORY_LOG, $log_msg, $file, $line);
}
...
function DebugMsg($log_msg, $file = '', $line = '')
{
    ...
    if($GLOBALS['Auth']->getSetting('debug_trans') == 'yes')
        QCore_History::writeHistory(HISTORY_LOG, $log_msg, $file, $line);
}
...
function setLanguage()
{
    ...
    if($_REQUEST['lang'] != '' && file_exists(LANG_PATH.$_REQUEST['lang'].'.php'))
       $lang = $_REQUEST['lang'];
    
    if($lang == '' && $_SESSION[SESSION_PREFIX.'lang'] != '' && file_exists(LANG_PATH.$_SESSION[SESSION_PREFIX.'lang'].'.php'))
        $lang = $_SESSION[SESSION_PREFIX.'lang'];
    
    if($lang == '')
        $lang = $GLOBALS['Auth']->getSetting('default_lang');
...
function _rnd($value)
{
    $decimalPlaces = $GLOBALS['Auth']->getSetting('Acct_round_numbers');
