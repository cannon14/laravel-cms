...
class Affiliate_Scripts_Bl_PendingCommissionStatistics
{
    function getPendingCommissionStats($AffiliateID, 
                               $CampaignID, 
                               $d1='',$m1='',$y1='',$d2='',$m2='',$y2='',
                               $AccountID='',
                               $settings='')
    {
        ...
        $fieldArrays = Affiliate_Scripts_Bl_PendingCommissionStatistics::initFields();
        
        $UserData = Affiliate_Scripts_Bl_PendingCommissionStatistics::initResults($fieldArrays, $UserData);
        ...
        while(!$rs->EOF)
        {
            $UserData = Affiliate_Scripts_Bl_PendingCommissionStatistics::processRecords($UserData, $rs, $fieldArrays, $settings);
        ...
        //------------------------------------------------------------------------
        // compute bonuses
        $bonusLevels = array(	array("BRONZE Bonus",$UserData['revenue_sales_approved']*1.10),
        					array("SILVER Bonus",$UserData['revenue_sales_approved']*1.20),
        					array("GOLD Bonus",$UserData['revenue_sales_approved']*1.30),
        					array("PLATINUM Bonus",$UserData['revenue_sales_approved']*1.40));
        
        $UserData['bonusLevels'] = $bonusLevels;
		//------------------------------------------------------------------------
        // compute epc
        if($UserData['clicks'] == 0)
        {
            $UserData['epc'] = '0.00';
            $epcLevels = array(	array("BRONZE EPC",0),
        					array("SILVER EPC",0),
        					array("GOLD EPC",0),
        					array("PLATINUM EPC",0));
        
        }
        else
        {
            $UserData['epc'] = $UserData['revenue_sales_approved']/$UserData['clicks'];
            $epcLevels = array(	array("BRONZE EPC",$UserData['epc']*1.10),
        					array("SILVER EPC",$UserData['epc']*1.20),
        					array("GOLD EPC",$UserData['epc']*1.30),
        					array("PLATINUM EPC",$UserData['epc']*1.40));
        
        }
        $UserData['epcLevels'] = $epcLevels;
		
        
        //------------------------------------------------------------------------
        // round results
        $UserData = Affiliate_Scripts_Bl_PendingCommissionStatistics::roundResults($fieldArrays, $UserData);
    ...
    function processRecords($UserData, $rs, $fieldArrays, $settings)
    {
        ...
        {
            $UserData = Affiliate_Scripts_Bl_PendingCommissionStatistics::processNormalStats($fieldArrays['clicksFields'], $rs->fields, $UserData, $settings['Aff_support_click_commissions']);
        }
        else if($rs->fields['transtype'] == TRANSTYPE_SALE)
        {
            $UserData = Affiliate_Scripts_Bl_PendingCommissionStatistics::processNormalStats($fieldArrays['salesFields'], $rs->fields, $UserData, $settings['Aff_support_sale_commissions']);
        }
        else if($rs->fields['transtype'] == TRANSTYPE_LEAD)
        {
            $UserData = Affiliate_Scripts_Bl_PendingCommissionStatistics::processNormalStats($fieldArrays['leadsFields'], $rs->fields, $UserData, $settings['Aff_support_lead_commissions']);
        }
        else if($rs->fields['transtype'] == TRANSTYPE_RECURRING)
        {
            $UserData = Affiliate_Scripts_Bl_PendingCommissionStatistics::processNormalStats($fieldArrays['recurringFields'], $rs->fields, $UserData, $settings['Aff_support_recurring_commissions']);
        }
        else if($rs->fields['transtype'] == TRANSTYPE_CPM)
        {
            $UserData = Affiliate_Scripts_Bl_PendingCommissionStatistics::processNormalStats($fieldArrays['cpmFields'], $rs->fields, $UserData, $settings['Aff_support_cpm_commissions']);
        }
        else if($rs->fields['transtype'] == TRANSTYPE_SIGNUP)
        {
            $UserData = Affiliate_Scripts_Bl_PendingCommissionStatistics::processNormalStats($fieldArrays['signupFields'], $rs->fields, $UserData, $settings['Aff_support_signup_commissions']);
        }
        else if($rs->fields['transtype'] == TRANSTYPE_REFERRAL)
        {
            $UserData = Affiliate_Scripts_Bl_PendingCommissionStatistics::processNormalStats($fieldArrays['referralFields'], $rs->fields, $UserData, $settings['Aff_support_referral_commissions']);
        }
        else if($rs->fields['transtype'] == TRANSTYPE_BONUS)
        {
            $UserData = Affiliate_Scripts_Bl_PendingCommissionStatistics::processNormalStats($fieldArrays['bonusFields'], $rs->fields, $UserData, $settings['Aff_support_sale_commissions']);
        }
        return $UserData;
    }
