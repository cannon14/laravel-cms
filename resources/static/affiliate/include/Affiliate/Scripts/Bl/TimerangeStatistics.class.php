...
class Affiliate_Scripts_Bl_TimerangeStatistics
{
                               ...
                               $settings='')
    {
        ...
        $fieldArrays = Affiliate_Scripts_Bl_TimerangeStatistics::initFields();
        
        $UserData = Affiliate_Scripts_Bl_TimerangeStatistics::initResults($fieldArrays, $UserData);
        ...
        $sql = "select a.userid, sum(all_imps_count) as today_imps, sum(unique_imps_count) as today_unique_imps ".
               ...
               "where i.bannerid=b.bannerid and c.campaignid=b.campaignid and i.affiliateid=a.userid and a.deleted=0 AND userid IN (SELECT affiliateid FROM cs_affiliateaccess WHERE userid="._q($GLOBALS['Auth']->getUserID()).") AND a.rstatus=".AFFSTATUS_APPROVED;
        if($filterByDate)
        {
        ...
        while(!$rs->EOF)
        {
            $UserData = Affiliate_Scripts_Bl_TimerangeStatistics::processRecords($UserData, $rs, $fieldArrays, $settings);
        ...
        //------------------------------------------------------------------------
        // round results
        $UserData = Affiliate_Scripts_Bl_TimerangeStatistics::roundResults($fieldArrays, $UserData);
    ...
    function processRecords($UserData, $rs, $fieldArrays, $settings)
    {
        ...
        {
            $UserData = Affiliate_Scripts_Bl_TimerangeStatistics::processNormalStats($fieldArrays['clicksFields'], $rs->fields, $UserData, $settings['Aff_support_click_commissions']);
        }
        else if($rs->fields['transtype'] == TRANSTYPE_SALE)
        {
            $UserData = Affiliate_Scripts_Bl_TimerangeStatistics::processNormalStats($fieldArrays['salesFields'], $rs->fields, $UserData, $settings['Aff_support_sale_commissions']);
        }
        else if($rs->fields['transtype'] == TRANSTYPE_LEAD)
        {
            $UserData = Affiliate_Scripts_Bl_TimerangeStatistics::processNormalStats($fieldArrays['leadsFields'], $rs->fields, $UserData, $settings['Aff_support_lead_commissions']);
        }
        else if($rs->fields['transtype'] == TRANSTYPE_RECURRING)
        {
            $UserData = Affiliate_Scripts_Bl_TimerangeStatistics::processNormalStats($fieldArrays['recurringFields'], $rs->fields, $UserData, $settings['Aff_support_recurring_commissions']);
        }
        else if($rs->fields['transtype'] == TRANSTYPE_CPM)
        {
            $UserData = Affiliate_Scripts_Bl_TimerangeStatistics::processNormalStats($fieldArrays['cpmFields'], $rs->fields, $UserData, $settings['Aff_support_cpm_commissions']);
        }
        else if($rs->fields['transtype'] == TRANSTYPE_SIGNUP)
        {
            $UserData = Affiliate_Scripts_Bl_TimerangeStatistics::processNormalStats($fieldArrays['signupFields'], $rs->fields, $UserData, $settings['Aff_support_signup_commissions']);
        }
        else if($rs->fields['transtype'] == TRANSTYPE_REFERRAL)
        {
            $UserData = Affiliate_Scripts_Bl_TimerangeStatistics::processNormalStats($fieldArrays['referralFields'], $rs->fields, $UserData, $settings['Aff_support_referral_commissions']);
        }
