...
class Affiliate_Merchants_Bl_Merchant
{
    ...
    function getMerchantsAsArray()
    {
        $sql = 'select * from rt_merchants m where m.deleted=0';
        $rs = QCore_Sql_DBUnit::execute($sql, __FILE__, __LINE__);
        ...
        {
            $temp = array();
            $temp['merchantId'] = $rs->fields['merchantId'];
            $temp['name'] = $rs->fields['longName'];
            $temp['type'] = $rs->fields['type'];
            $thisList[$rs->fields['merchantId']] = $temp;
