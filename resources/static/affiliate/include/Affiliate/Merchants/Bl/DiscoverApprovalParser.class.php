...
class Affiliate_Merchants_Bl_DiscoverApprovalParser extends Affiliate_Merchants_Bl_Parser {
    ...
    function parseData($path, $fileErrorPath, $archivePath, $cleanPath, $sqlErrorPath, $filename) {
		...
		else
		{
			...
			while (!feof ($fd)) {
				...
				{
					/**
					 * We only get paid on approvals with Discover Business so we only want to
					 * take records where the Decision column equals 1.
					 * 
					 */
