...
class Affiliate_Merchants_Bl_Pulse360Parser extends Affiliate_Merchants_Bl_ExpenseParserNew {
    ...
    {
    	if ((!strstr($buffer[0], 'Topic')) || (!strstr($buffer[1], 'Clicks')) || (!strstr($buffer[2], 'PageViews')) || (!strstr($buffer[3], 'Avg_Pos')) || (!strstr($buffer[4], 'CTR')) || (!strstr($buffer[5], 'Cost')) || (!strstr($buffer[6], 'Avg_CPC')))
    	{
			return false;
    	} else {
    		return true;
