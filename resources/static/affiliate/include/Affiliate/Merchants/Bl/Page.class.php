...
class Affiliate_Merchants_Bl_Page
{
    ...
    {
        $sql = 'select * from pages p where p.deleted=0 order by page_name';
        $rs = QCore_Sql_DBUnit::execute($sql, __FILE__, __LINE__);
        ...
        {
            $temp = array();
            $temp['pageId'] = $rs->fields['page_id'];
            $temp['name'] = $rs->fields['page_name'];
            $thisList[$rs->fields['page_id']] = $temp;
