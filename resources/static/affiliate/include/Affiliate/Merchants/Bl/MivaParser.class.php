...
class Affiliate_Merchants_Bl_MivaParser extends Affiliate_Merchants_Bl_ExpenseParserNew {
    ...
    function parseData($path, $fileErrorPath, $archivePath, $cleanPath, $sqlErrorPath, $filename) {
		...
		while (!feof ($fd)) {
			...
			// check for empty row - DO NOT take transactions with zero clicks
    ...
    {
    	if ((!strstr($buffer[0], 'Date')) || (!strstr($buffer[1], 'Number of Clicks')) || (!strstr($buffer[2], 'Dollars Spent')) || (!strstr($buffer[3], 'Average Cost Per Click')))
    	{
			return false;
    	} else {
    		return true;
