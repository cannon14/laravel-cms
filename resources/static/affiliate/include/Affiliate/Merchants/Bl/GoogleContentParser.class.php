...
class Affiliate_Merchants_Bl_GoogleContentParser extends Affiliate_Merchants_Bl_ExpenseParserNew {
		...
		while (!feof ($fd)) {
			...
			if (($buffer[5] != "") && ($buffer[8] != "") && ($buffer[2] != ""))
			{
				if ($i > 1) {
					...
					$rowArr["total_expense"] = $this->cleanFinancial($buffer[8]);
					$rowArr["quantity"] = $this->cleanFinancial($buffer[5]);
    ...
    {
    	if ((!strstr($buffer[0], 'Campaign')) || (!strstr($buffer[1], 'Ad Group')) || (!strstr($buffer[2], 'Destination URL')) || (!strstr($buffer[3], 'Ad Distribution')) || (!strstr($buffer[4], 'Impressions')) || (!strstr($buffer[5], 'Clicks')) || (!strstr($buffer[6], 'CTR')) || (!strstr($buffer[7], 'Avg CPC')) || (!strstr($buffer[8], 'Cost')) || (!strstr($buffer[9], 'Avg Position')))
    	{
			return false;
    	} else {
    		return true;
