...
class Affiliate_Merchants_Bl_Keyword
{
    ...
    {
        $sql = 'select * from rt_keywords where deleted=0 ';
        
        if($_REQUEST['sortby'] != '' && in_array($_REQUEST['sortby'], $a)) {
            $orderby = ' order by '.$_REQUEST['sortby'].' '.$_REQUEST['sortorder'];
        }
        else {
            $orderby = ' order by ordering desc, keywordId '; 
        }
        $limitby = ' limit 1000';
        
        
        //echo $sql;
        $rs = QCore_Sql_DBUnit::execute($sql.$orderby.$limitby, __FILE__, __LINE__);
        ...
        while(!$rs->EOF)
        {
            ...
            $thisList[$rs->fields['keywordId']] = $temp;
