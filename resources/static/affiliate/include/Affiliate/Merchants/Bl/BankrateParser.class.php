...
class Affiliate_Merchants_Bl_BankrateParser extends Affiliate_Merchants_Bl_ExpenseParserNew {
		...
		while (!feof ($fd)) {
			...
			if (($buffer[7] != "") || ($buffer[4] != ""))
			{
				if ($i > 1) {
					...
					$urlInfo = $this->parseUrl($buffer[4]);
					...
					$rowArr["quantity"] = $this->cleanFinancial($buffer[6]);
