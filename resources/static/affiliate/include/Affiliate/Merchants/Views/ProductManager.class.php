...
class Affiliate_Merchants_Views_ProductManager extends QUnit_UI_TemplatePage
{
    ...
    function processEditCampaign()
    {
        ...
        {
            // save change
            $ret = Affiliate_Merchants_Bl_Campaign::updateCampaign($campaignProcessedData);
    ...
    function processCampaign($data)
    {
        ...
        if(strlen($_POST['desturl']) > 223){
            QUnit_Messager::setErrorMessage('URL must be less than 224 characters!');        
        }
    ...
    function processAddCampaign()
    {
        ...
        $bannerid = preg_replace('/[\'\"]/', '', $_POST['banner_id']);
        $merchantid = preg_replace('/[\'\"]/', '', $_POST['merchant_id']);
        
        //Banner stuff
        $pdesturl 	= preg_replace('/[\'\"]/', '', $_POST['desturl']);
        $psourceurl = preg_replace('/[\'\"]/', '', $_POST['sourceurl']);
        $pdesc 		= preg_replace('/[\'\"]/', '', $_POST['desc']);
        
        // epc stuff.  - mz 1/8/07
        $epc_rate_override = preg_replace('/[\'\"]/', '', $_POST['epc_rate_override']);
        $use_override = preg_replace('/[\'\"]/', '', $_POST['use_override']);
        ...
        if($_POST['banner_id'] != '' && $this->checkBannerIdExists($_POST['banner_id']))
            QUnit_Messager::setErrorMessage('The product ID already exists.');            
        ...
        checkCorrectness($_POST['desturl'], $pdesturl, 'Default Ad Banner Destination URL: ', CHECK_EMPTYALLOWED);
        $psourceurl = 'Default AdUnit';
...
//            QUnit_Messager::setErrorMessage(L_G_COMMISSIONTYPEMUSTBECHOSEN);
        ...
        if(QUnit_Messager::getErrorMessage() == '')
        {
            ...
            $ret = Affiliate_Merchants_Bl_Campaign::insert($params);
            
            if(!$ret) return false;
            
            // added code to insert epc data when the campaign is inserted.  - mz 1/8/07           
            $ret = Affiliate_Merchants_Bl_Campaign::insertNewEpcData($bannerid, $epc_rate_override, $use_override);
    ...
    function loadCampaignInfo()
    {
        ...
        $data = Affiliate_Merchants_Bl_Campaign::load($params);
        ...
        $_POST['reference_id'] = $data['reference_id'];
        $_POST['merchant_id'] = $data['merchant_id'];
        
        $_POST['banner_id'] = $data['bannerid'];
        $_POST['desturl'] = $data['destinationurl'];
...
//        $this->clearTempContent();
    ...
    function drawFormAddCampaign()
    {
        ...
        if(count($merchants) < 1){
        	QUnit_Messager::setErrorMessage('There are no merchants defined.  Create a merchant before you create a product.');
        	return true;
        }
    ...
    {
        $orderby = '';
        $a = array("name", "dateinserted", "commtype", "weburl", "campaignid", "short_name", "reference_id");
        ...
        $where = "where c.deleted=0 and accountid="._q($GLOBALS['Auth']->getAccountID()) . " AND campaign_type= " . _q('PRODUCT');
...
//                $where .= " and (commtype in ".$commtypes.")";
        ...
        $sql = "select c.*, m.short_name, b.bannerid from wd_pa_campaigns as c LEFT JOIN wd_pa_banners AS b USING (campaignid) LEFT join merchants as m USING(merchant_id) $where $orderby";
        ...
        while(!$rs->EOF)
        {
            ...
            $temp['merchant_name'] = $rs->fields['short_name'];
            $temp['reference_id'] = $rs->fields['reference_id'];
            $temp['bannerid'] = $rs->fields['bannerid'];
    ...
    function checkProductRefExists($productRefId)
    {
        $sql = 'select * from wd_pa_campaigns '.
        'where deleted=0 and product_ref_id='._q($productRefId).
        '  and accountid='._q($GLOBALS['Auth']->getAccountID());
    ...
    function checkBannerIdExists($bannerId)
    {
        $sql = 'select * from wd_pa_banners '.
        'where deleted=0 and bannerid='._q($bannerId);
    ...
    {
        $sql = 'select * from wd_pa_campaigns c where c.deleted=0 and accountid='._q($GLOBALS['Auth']->getAccountID()).' AND c.campaign_type = ' ._q('PRODUCT'). 'order by name';
        $rs = QCore_Sql_DBUnit::execute($sql, __FILE__, __LINE__);
