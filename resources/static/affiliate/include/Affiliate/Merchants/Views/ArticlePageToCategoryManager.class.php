...
class Affiliate_Merchants_Views_ArticlePageToCategoryManager extends QUnit_UI_ListPage
{
    var $campCategory;
...
//        $this->modulePermissions['view'] = 'aff_trans_transactions_view';
    ...
    function process()
    {
        ...
        if(!empty($_REQUEST['sortableListsSubmitted']))
        {
			...
			foreach($orderArray as $item) {
				$sql[] = "INSERT INTO rt_articlepagecategorymap (rank, categoryId, cardpageId) VALUES (" . _q($item['order']) . ", " . _q($_REQUEST['siteId']) . ","  . _q($item['element']) . ")";
			}
	...
	function processAddPage(){
		$sql = "SELECT MAX(rank) FROM rt_articlepagecategorymap WHERE categoryId = " . _q($_REQUEST['categoryId']);
		
		$rs = QCore_Sql_DBUnit::execute($sql, __FILE__, __LINE__);
		$max = $rs->fields['MAX(rank)'];
		if($max == '')
			$max = 0;
		$sql = "DELETE FROM rt_articlepagecategorymap WHERE cardpageId=" . _q($_REQUEST['pageID']) . " AND categoryId=" . _q($_REQUEST['categoryId']);
		
		$rs = QCore_Sql_DBUnit::execute($sql, __FILE__, __LINE__);
		$sql = "INSERT INTO rt_articlepagecategorymap (rank, categoryId, cardpageId) VALUES (" . _q(($max+1)) . ", " . _q($_REQUEST['categoryId']) . ","  . _q($_REQUEST['pageID']) . ")";
		
		$rs = QCore_Sql_DBUnit::execute($sql, __FILE__, __LINE__);
		
		$this->redirect('Affiliate_Merchants_Views_ArticlePageToCategoryManager&type=all&siteId='.$_REQUEST['categoryId']);

	}    
	
	function processRemovePage(){
		$sql = "SELECT * FROM rt_articlepagecategorymap WHERE categoryId=" . _q($_REQUEST['categoryId']) . " AND cardpageId != " . _q($_REQUEST['cardpageId']) . " ORDER BY rank";
		
		$rs = QCore_Sql_DBUnit::execute($sql, __FILE__, __LINE__);
		$sql = "DELETE FROM rt_articlepagecategorymap WHERE categoryId=" . _q($_REQUEST['categoryId']);
		
		QCore_Sql_DBUnit::execute($sql, __FILE__, __LINE__);
		$count = 1;
		while(!$rs->EOF){
			$sql = "INSERT INTO rt_articlepagecategorymap (rank, categoryId, cardpageId) VALUES (" . _q($count) . ", " . _q($rs->fields['categoryId']) . ","  . _q($rs->fields['cardpageId']) . ")";
	...
	function updateOrder($sql){
		$deletesql = "DELETE from rt_articlepagecategorymap where categoryId = " . _q($_REQUEST['siteId']);
    ...
    function showTransactions($exportToCsv)
    {
		...
		$this->addContent('assignarticlecategories_list'); 
	...
	function getUnassignedRecords()
    {
		...
		if(count($assigned) > 0){
			...
			$sqlIds = "('')";
		$sql = "select * from rt_cardpages as c where c.type = 1 AND !(c.cardpageId in " . $sqlIds . ") and c.deleted != 1";  
		//echo "SQL: ".$sql;
        $rs = QCore_Sql_DBUnit::execute($sql, __FILE__, __LINE__);
        if (!$rs)
        {
	...
	function getAssignedRecords()
    {
        ...
        //------------------------------------------------
        // get records
        $sql = "select * from rt_pagedetails as d, rt_cardpages as s, rt_articlepagecategorymap as m, rt_categories as c where s.type = 1 AND (d.deleted != 1 AND s.deleted != 1) AND d.pageDetailVersion = -1 AND (d.cardpageId =  s.cardpageId) AND m.categoryId =" . _q($_REQUEST['siteId']) . " and (m.cardpageId=s.cardpageId) and (m.categoryId=c.categoryId) and s.deleted != 1 ORDER BY m.rank ASC";
		//echo "<br>SQL: ".$sql;
        $rs = QCore_Sql_DBUnit::execute($sql, __FILE__, __LINE__);
        if (!$rs)
        {
