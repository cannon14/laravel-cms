...
class Affiliate_Merchants_Views_CampaignManager extends QUnit_UI_TemplatePage
{
...
//                $where .= " and (commtype in ".$commtypes.")";
    ...
    function getCampaignsAsArray($orderByMerchant = false)
    {
        $sql = 'SELECT b.bannerid, m.long_name, c.campaignid, c.name, c.commtype, m.long_name AS merchant_name 
					FROM wd_pa_campaigns AS c LEFT JOIN merchants AS m USING (merchant_id) 
					LEFT JOIN wd_pa_banners AS b USING (campaignid) 
					WHERE c.campaign_type = \'PRODUCT\' AND c.deleted != 1 ';

        if(!$orderByMerchant) {
		    $sql .= ' ORDER BY c.name ASC';
		}else { 
		    $sql .= ' ORDER BY m.long_name ASC, c.name ASC';
        }
        ...
        {
            $temp = array();
            $temp['merchantname'] = $rs->fields['merchant_name'];
