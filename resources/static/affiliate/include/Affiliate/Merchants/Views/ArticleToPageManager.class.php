...
class Affiliate_Merchants_Views_ArticleToPageManager extends QUnit_UI_ListPage
{
...
//        $this->modulePermissions['view'] = 'aff_trans_transactions_view';
    ...
    function process()
    {
        ...
        if(!empty($_REQUEST['sortableListsSubmitted']))
        {
			...
			foreach($orderArray as $item) {
				$sql[] = "INSERT INTO rt_articlepagemap (rank, cardpageId, articleId) VALUES (" . _q($item['order']) . ", " . _q($_REQUEST['siteId']) . ","  . _q($item['element']) . ")";
			}
	...
	function processAddCard(){
		$sql = "SELECT MAX(rank) FROM rt_articlepagemap WHERE cardpageId = " . _q($_REQUEST['siteId']);
		$rs = QCore_Sql_DBUnit::execute($sql, __FILE__, __LINE__);
		$max = $rs->fields['MAX(rank)'];
		if($max == '')
			$max = 0;
		$sql = "DELETE FROM rt_articlepagemap WHERE articleId=" . _q($_REQUEST['cardId']) . " AND cardpageId=" . _q($_REQUEST['siteId']);
		$rs = QCore_Sql_DBUnit::execute($sql, __FILE__, __LINE__);
		$sql = "INSERT INTO rt_articlepagemap (rank, cardpageId, articleId) VALUES (" . _q(($max+1)) . ", " . _q($_REQUEST['siteId']) . ","  . _q($_REQUEST['cardId']) . ")";
		$rs = QCore_Sql_DBUnit::execute($sql, __FILE__, __LINE__);
		
	}
	
	function processRemoveCard(){
		$sql = "SELECT * FROM rt_articlepagemap WHERE cardpageId=" . _q($_REQUEST['siteId']) . " AND articleId!= " . _q($_REQUEST['cardId']) . " ORDER BY rank";
		//QUnit_Messager::setErrorMessage("processRemoveCard " . $sql);
		$rs = QCore_Sql_DBUnit::execute($sql, __FILE__, __LINE__);
		$sql = "DELETE FROM rt_articlepagemap WHERE cardpageId=" . _q($_REQUEST['siteId']);
		//QUnit_Messager::setErrorMessage("processRemoveCard " . $sql);
		QCore_Sql_DBUnit::execute($sql, __FILE__, __LINE__);
		$count = 1;
		while(!$rs->EOF){
			$sql = "INSERT INTO rt_articlepagemap (rank, cardpageId, articleId) VALUES (" . _q($count) . ", " . _q($rs->fields['cardpageId']) . ","  . _q($rs->fields['articleId']) . ")";
			//QUnit_Messager::setErrorMessage("processRemoveCard " . $sql);
			QCore_Sql_DBUnit::execute($sql, __FILE__, __LINE__);
			$count ++;
			$rs->MoveNext();
	...
	function updateOrder($sql){
		$deletesql = "DELETE from rt_articlepagemap where cardpageId = " . _q($_REQUEST['siteId']);
		//QUnit_Messager::setErrorMessage("updateOrder " . $deletesql);
		$rs = QCore_Sql_DBUnit::execute($deletesql, __FILE__, __LINE__);
		if($sql != ''){
			foreach($sql as $currentSql){
				//QUnit_Messager::setErrorMessage("updateOrder " . $currentSql);
    ...
    function showTransactions($exportToCsv)
    {
		...
		**/
		...
		$this->addContent('assignarticles_list');
	...
	function getUnassignedSubCats()
    {
		...
		if(count($assigned) > 0){
			...
			$sqlIds = "('')";
		$sql = "select * from rt_articles as c WHERE  c.subCat = 1 AND c.articleId not in " . $sqlIds . " and c.deleted != 1";
    ...
    {	
        ...
        // get records
        		$sql = "select * from 
				rt_cardpages as p, rt_articlepagemap as m, rt_articles as c 		
				where  c.subCat = 1 		
				AND m.cardpageId = " . _q($_REQUEST['siteId']) . "		
				AND (m.cardpageId=p.cardpageId) 		
				AND (m.articleId=c.articleId) 		
				AND c.deleted != 1 	
				ORDER BY m.rank ";
	...
	function getUnassignedRecords()
    {
		...
		if(count($assigned) > 0){
			...
			$sqlIds = "('')";
		$sql = "select * from rt_articles as c WHERE c.subCat = 0 AND c.articleId not in " . $sqlIds . " and c.deleted != 1";
