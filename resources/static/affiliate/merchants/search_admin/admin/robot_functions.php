...
function phpdigGetSiteFromUrl($id_connect,$url,$linksper,$linksper_flag,$limit,$limit_flag,$usetable) {
    ...
    if (mysql_num_rows($result) > 0) {
        ...
        if ($subpu['ok'] == 1) {
            ...
            $query_tempspider = "INSERT INTO ".PHPDIG_DB_PREFIX."tempspider (site_id,file_path,path) VALUES ('$site_id','".$subpu['file_path']."','".$subpu['path']."')";
            mysql_query($query_tempspider,$id_connect);
        }
    }
    else {
         ...
         if ($subpu['ok'] == 1) {
            ...
            $query = "INSERT INTO ".PHPDIG_DB_PREFIX."tempspider SET file_path='".$subpu['file_path']."',path='".$subpu['path']."',level=0,site_id='$site_id'";
            mysql_query($query,$id_connect);
         }
    }
...
if (!$fp) {
  ...
  ."User-Agent: PhpDig/".PHPDIG_VERSION." (+http://www.creditcards.com/robot.php)".END_OF_LINE_MARKER.END_OF_LINE_MARKER;
...
function phpdigSetHeaders($cookiesToSend=array(),$path='') {
     if (is_array($cookiesToSend) && count($cookiesToSend) > 0) {
         @ini_set('user_agent','PhpDig/'.PHPDIG_VERSION.' (+http://www.creditcards.com/robot.php)'.END_OF_LINE_MARKER.phpdigMakeCookies($cookiesToSend,$path));
     }
}
//=================================================
...
{
$test = parse_url($link['path'].$link['file_path']);

//test the exclude with robots.txt
if (phpdigReadRobots($exclude,$link['path'].$link['file_path']) == 1
    || isset($exclude['@ALL@'])
    ) {
    $link['ok'] = 0;
}
...
elseif (!isset($test['query'])
     && !eregi('[.][a-z0-9]{1,4}$',$link['path'].$link['file_path'])
     && $status = phpdigTestUrl($link['url'].$link['path'].$link['file_path'].'/','date',$cookies)
     && isset($status['status']) && $status['status'] == "HTML"
     ) {
      $link['path'] = ereg_replace ('/+$','/',$link['path'].$link['file_path'].'/');
      if ($link['path'] == '/') {
          $link['path'] = '';
      }
      $link['file_path'] = "";
...
$query_select = "SELECT spider_id FROM ".PHPDIG_DB_PREFIX."spider WHERE site_id=".(int)$site_id." AND path = '$path' AND file_path = '$file'";
...
if (!is_array($result_double)) {
    $requete = "INSERT INTO ".PHPDIG_DB_PREFIX."spider SET path='$path',file_path='$file',first_words='$first_words',upddate='$upddate',md5='$md5',site_id='$site_id',num_words='$num_words',last_modified='$lastmodified',filesize=".(int)$filesize;
...
$requete = "SELECT spider_id,
             ...
             WHERE spider.site_id='$site_id' AND spider.path = '$path' AND spider.file_path = '$file'";
...
function phpdigSpiderAddSite($id_connect,$url,$linksper,$linksper_flag,$limit,$limit_flag,$usetable) {
    ...
    if (mysql_num_rows($result) > 0) {
        list($site_id) = mysql_fetch_row($result);
        $query = "DELETE FROM ".PHPDIG_DB_PREFIX."tempspider WHERE site_id = $site_id AND file_path = '".$pu['query']."' and path = '".$pu['path']."'";
        mysql_query($query,$id_connect);
...
// Modified by Christian Lavender on March 28, 2006
...
function phpdigGetLogs($id_connect,$type='lastqueries',$timeframe='yesterday',$count='100') {
$result='';
$timeFrameString='';
switch ($timeframe) {
    case 'yesterday':
    	  $sdate = date('Y-m-d',strtotime('yesterday'));
		  $timeFrameString = "AND l_ts > '"+$sdate+"'";
		  break;
  	case 'thisweek':
		  ...
		  break;
  	case 'lastweek':
  		  $sdate = date('Y-m-d',strtotime('-1 week Monday'));
  		  $edate = date('Y-m-d',strtotime('last Monday'));
  		  
		  $timeFrameString = "AND l_ts > '"+$sdate+" AND l_ts < '"+$edate+"'";
		  break;
    case 'thismonth':
    	  $tmonth = date("m");
    	  $tyear = date("y");
		  $timeFrameString = "AND l_ts > '"+$tyear+"-"+$tmonth+"-01'";
		  break;
    case 'lastmonth':
    	  $ldate = strtotime('last month');
    	  $tmonth = date("m",$ldate);
    	  $tyear = date("y",$ldate);
		  $timeFrameString = "AND l_ts > '"+$tyear+"-"+$tmonth+"-01'";
		  break;
    case 'last3months':
    	  $ldate = strtotime('-3 months');
		  $tmonth = date("m",$ldate);
    	  $tyear = date("y",$ldate);
		  $timeFrameString = "AND l_ts > '"+$tyear+"-"+$tmonth+"-01'";
		  break;
    case 'last6months':
    	  $ldate = strtotime('-6 months');
    	  $tmonth = date("m",$ldate);
    	  $tyear = date("y",$ldate);
		  $timeFrameString = "AND l_ts > '"+$tyear+"-"+$tmonth+"-01'";
		  break;
    case 'yeartodate':
		  $timeFrameString = "AND l_ts > '"+$tyear+"-01-01' ";
		  break;
    case 'lastyear':
    	  $tyear = date("y");
    	  $lyear = $tyear-1;
		  $timeFrameString = "AND l_ts > '"+$lyear+"' ";
		  break;
...
switch ($type) {
    case 'mostkeys':
		 ...
		 $query = 'SELECT k.keyword ,sum(e.weight) as num
          ...
          WHERE k.key_id = e.key_id
          GROUP BY k.keyword
          ORDER BY num DESC LIMIT '.$_GET['count'].'';
          $result = phpdigMySelect($id_connect,$query);
    ...
    case 'mostpages':
	if ($_GET['count'] == 'all') {
          $query = 'SELECT CONCAT(st.site_url,s.path,s.file_path) as page,s.num_words
		 ...
		 $query = 'SELECT CONCAT(st.site_url,s.path,s.file_path) as page,s.num_words
          FROM '.PHPDIG_DB_PREFIX.'spider s, '.PHPDIG_DB_PREFIX.'sites st
          WHERE s.site_id = st.site_id
          ORDER BY num_words DESC LIMIT '.$_GET['count'].'';
          $result = phpdigMySelect($id_connect,$query);
	...
	if ($_GET['count'] == 'all') {
		...
		break;
		 ...
		 $query = 'SELECT l_includes as search_terms,
          ...
          '+$timeFrameString+'
          GROUP BY search_terms
          ORDER BY num_time DESC LIMIT '.$_GET['count'].'';
          $result = phpdigMySelect($id_connect,$query);
    break;
	}
   ...
   break;
		 ...
		 $query = 'SELECT count(l_id) as queries,
          ...
          GROUP BY with_terms, and_without
          HAVING average_results > 0
          ORDER BY average_results DESC LIMIT '.$_GET['count'].'';
          $result = phpdigMySelect($id_connect,$query);
   break;
		 }
    ...
    break;
		 ...
		 $query = 'SELECT count(l_id) as queries,
          ...
          AND l_includes <> \'\'
          GROUP BY with_terms, and_without
          ORDER BY queries DESC LIMIT '.$_GET['count'].'';
          $result = phpdigMySelect($id_connect,$query);
    break;
		 }
...
break;
		 ...
		 $query = 'SELECT DATE_FORMAT(l_ts,\'%Y-%m-%d %H:%i%:%S\') as date,
          ...
          l_time as search_time
          FROM '.PHPDIG_DB_PREFIX.'logs
          ORDER BY l_ts DESC LIMIT '.$_GET['count'].'';
          $result = phpdigMySelect($id_connect,$query);
break;
		 }
    ...
    break;
		 ...
		 $query = 'SELECT DATE_FORMAT(c_time,\'%Y-%m-%d %H:%i%:%S\') as date,
          ...
          c_val as link_query
          FROM '.PHPDIG_DB_PREFIX.'clicks
          ORDER BY c_time DESC LIMIT '.$_GET['count'].'';
          $result = phpdigMySelect($id_connect,$query);
    break;
		 }
