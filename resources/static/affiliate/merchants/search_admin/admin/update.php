...
elseif (!$verify[0]['locked']) {
  ...
  elseif ($exp) {
    ...
    $query = "INSERT INTO ".PHPDIG_DB_PREFIX."tempspider (site_id,file_path,path) SELECT site_id,file_path,path FROM ".PHPDIG_DB_PREFIX."spider WHERE site_id=$site_id $andpath";
    mysql_query($query,$id_connect);
...
<head>
<title>CreditCards.com : <?php phpdigPrnMsg('update') ?> </title>
...
<a href="index.php" target="_parent">[<?php phpdigPrnMsg('back') ?>]</a> <?php phpdigPrnMsg('to_admin') ?>.
</body>
</html>