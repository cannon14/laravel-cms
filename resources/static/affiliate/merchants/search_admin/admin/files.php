...
if (is_array($verify) && !$verify[0]['locked'] && $spider_id) {
     ...
     $query = "SELECT site_id,path,file_path FROM ".PHPDIG_DB_PREFIX."spider where spider_id=$spider_id";
     ...
     if ($spider)  {
         ...
         $query = "INSERT INTO ".PHPDIG_DB_PREFIX."tempspider SET site_id=$site_id,path='$path',file_path='$file'";
         $result_id = mysql_query($query,$id_connect);
...
if ($site_id) {
  ...
  $query = "SELECT file_path,spider_id FROM ".PHPDIG_DB_PREFIX."spider WHERE site_id=$site_id AND path like '$path' ORDER by file_path";
  $result_id = mysql_query($query,$id_connect);
