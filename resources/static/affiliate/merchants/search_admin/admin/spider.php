...
if (isset($mode) && isset($site_id) && $mode == 'small') {
    print '<br /><a href="stop_spider.php?stop=1" target="_parent">['.phpdigMsg('StopSpider').']</a> '.phpdigMsg('spidering').'.';
}
else {
print '<br /><a href="stop_spider.php?stop=1">['.phpdigMsg('StopSpider').']</a> '.phpdigMsg('spidering').'.';
}
?></h3>
...
@ini_set('user_agent','PhpDig/'.PHPDIG_VERSION.' (+http://www.creditcards.com/robot.php)');
  ...
  while ($site_datas = array_pop($list_sites)) {
    ...
    if (is_array($verify)) {
        ...
        if (!(LIMIT_TO_DIRECTORY) && ($mode != 'small')) {
          if ($links_per_lev == 0) {
             $query_tempspider = "INSERT INTO ".PHPDIG_DB_PREFIX."tempspider (site_id,file_path,path) SELECT site_id,file_path,path FROM ".PHPDIG_DB_PREFIX."spider WHERE site_id=$site_id $andmore_tempspider";
             ...
             elseif (($links_per_lev > $query_count_num) && ($flag_for_inserts_check1 == 0)) {
               ...
               $query_tempspider = "INSERT INTO ".PHPDIG_DB_PREFIX."tempspider (site_id,file_path,path) SELECT site_id,file_path,path FROM ".PHPDIG_DB_PREFIX."spider WHERE site_id=$site_id $andmore_tempspider LIMIT $level_lim";
               mysql_query($query_tempspider,$id_connect);
             }
          }
        }
        ...
        while($level <= $limit) {
           ...
           if (mysql_num_rows($result_id_stop) > 0) {
              ...
              $query = "SELECT id,path,file_path,indexed FROM ".PHPDIG_DB_PREFIX."tempspider WHERE level = $level AND indexed = 0 AND site_id=$site_id AND error = 0 limit 1";
              ...
              if ($n_links > 0) {
                   while ($new_links = mysql_fetch_array($result_id)) {
                        ...
                        $result_id_stop = mysql_query($query_check_stop,$id_connect);
                        if (mysql_num_rows($result_id_stop) > 0) {
                            $query = "SELECT id,path,file_path,indexed FROM ".PHPDIG_DB_PREFIX."tempspider WHERE level = -1";
                            $result_id = mysql_query($query,$id_connect);
                        }
                        else {
                        ...
                        if ($exists_spider_id == 0 || $upddate < $date || ($force_first_reindex == 1 && ($level==0 || $already_indexed==0))) {
                           ...
                           if (is_array($result_test_http) && !in_array($result_test_http['status'],array('NOHOST','NOFILE','LOOP','NEWHOST'))) {
                               ...
                               if ($tested_url['path'] != $temp_path || $tested_url['file_path'] != $temp_file ) {
                                   $temp_path = $tested_url['path'];
                                   $temp_file = $tested_url['file_path'];
                                   $query = "UPDATE ".PHPDIG_DB_PREFIX."tempspider SET path='$temp_path', file_path='$temp_file', WHERE id=$tempspider_id";
                                   mysql_query($query,$id_connect);
                                   $url_indexing = $full_url.$temp_path.$temp_file;
                                   $url_indexing = ereg_replace("[?]$","",$url_indexing);
                                   $url_print = $url.$temp_path.$temp_file;
                               ...
                               $query = "UPDATE ".PHPDIG_DB_PREFIX."tempspider "
                                       ...
                                       ."OR site_id = $site_id AND path LIKE '$temp_path' AND file_path LIKE '$temp_file'";
                               mysql_query($query,$id_connect);
                           }
                        }
                        else {
                        ...
                        if (isset($urls) && is_array($urls)) {
                            ...
                            if (mysql_num_rows($result_id_stop) > 0) {
                               ...
                               if (isset($lien['newhost'])) { 
                                   if (PHPDIG_IN_DOMAIN == true && phpdigCompareDomains($lien['the_http_scheme'].'://'.$lien['newhost'].$lien['path'].$lien['file_path'],$url)) {
                                     $added_site = phpdigSpiderAddSite($id_connect,$lien['the_http_scheme'].'://'.$lien['newhost'].$lien['path'].$lien['file_path'],$linksper,$linksper_flag,$limit,$limit_flag,$usetable);
                                     ...
                                     if (!$site_exists && is_array($added_site)) {
                                       if ($added_site['site_id'] != "zzz") {
                                         print 'Ok for '.$lien['the_http_scheme'].'://'.$lien['newhost'].$lien['path'].$lien['file_path'].' (site_id:'.$added_site['site_id'].')'.$br;
                               ...
                               //not an apache fancy index (with sorts by columns && not a newhost)
                               else if (!isset($apache_indexes[$lien['file_path']])) {
                                  $exists = 0;
                                  $exists_temp_spider = 0;

                                  if (!get_magic_quotes_runtime()) {
                                      $lien['path'] = addslashes($lien['path']);
                                      $lien['file_path'] = addslashes($lien['file_path']);
                                  }

                                  //is this link already in temp table ?
                                  $query = "SELECT count(*) as num FROM ".PHPDIG_DB_PREFIX."tempspider WHERE path like '".$lien['path']."' AND file_path like '".$lien['file_path']."' AND site_id='$site_id'";
                                  ...
                                  $query = "SELECT count(*) as num FROM ".PHPDIG_DB_PREFIX."spider WHERE path like '".$lien['path']."' AND file_path like '".$lien['file_path']."' AND site_id='$site_id' $andmore";
                                  ...
                                  if (($exists_temp_spider < 1) && ($cur_link['ok'])) {
                                    ...
                                    settype($cur_link['file_path'],'string');

                                    $values =  "('".$cur_link['path']."','".$cur_link['file_path']."',".($level+1).",$site_id,$s_error)";
                                    
                                      if ($links_per_lev == 0) {
                                        $query = "INSERT INTO ".PHPDIG_DB_PREFIX."tempspider (path, file_path, level, site_id, error) VALUES $values";
                                        mysql_query($query,$id_connect);
                                      }
                                      else {
...
<?php
if (isset($mode) && isset($site_id) && $mode == 'small') {
    print '<br /><a href="update_frame.php?site_id='.$site_id.'" target="_parent">['.phpdigMsg('back').']</a> '.phpdigMsg('to_update').'.';
	print '<br /><a href="index.php" target="_parent">['.phpdigMsg('back').']</a> '.phpdigMsg('to_admin').'.';
}
else {
print '<br /><a href="index.php">['.phpdigMsg('back').']</a> '.phpdigMsg('to_admin').'.';
}
?>
</body>
</html>
