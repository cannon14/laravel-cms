...
if ((is_array($final_result)) && (count($final_result) > 0)) {
    ...
    for ($n = 1; $n <= $n_end; $n++) {
        ...
        if ($n >= $n_start) {
             ...
             $query = "SELECT sites.site_url, sites.port, spider.path,spider.file_path,spider.first_words,sites.site_id,spider.spider_id,spider.last_modified,spider.md5 "
             ...
             $url = eregi_replace("([".$phpdig_words_chars[PHPDIG_ENCODING]."])[/]{2,}","\\1/",urldecode($content['site_url'].$content['path'].$content['file_path']));

             $js_url = urlencode(eregi_replace("^[a-z]{3,5}://","",$url));

             $url = str_replace("\"","%22",str_replace("'","%27",str_replace(" ","%20",trim($url))));

             $l_site = "<a href='".SEARCH_PAGE."?refine=1&amp;query_string=".urlencode($my_query_string_link)."&amp;site=".$content['site_id']."&amp;limite=$limite&amp;option=$option'>".htmlspecialchars(urldecode($content['site_url']),ENT_QUOTES)."</a>";
             if ($content['path']) {
                  $content['path'] = urlencode(urldecode($content['path']));
                  $content2['path'] = htmlspecialchars(urldecode($content['path']),ENT_QUOTES);
                  $l_path = ", ".phpdigMsg('this_path')." : <a href='".SEARCH_PAGE."?refine=1&amp;query_string=".urlencode($my_query_string_link)."&amp;site=".$content['site_id']."&amp;path=".$content['path']."&amp;limite=$limite&amp;option=$option' >".$content2['path']."</a>";
             }
             else {
                  $content2['path'] = "";
                  $l_path="";
             ...
             $table_results[$n] = array (
                    ...
                    'page_link' => "<a href=\"".$url."\" onmousedown=\"return clickit(".$n.",'".$js_url."')\" target=\"".LINK_TARGET."\" >".$title."</a>",
    ...
    if ($lim_start > 0) {
        ...
        $nav_bar .= "<a href=\"$previous_link\" >&lt;&lt;".phpdigMsg('previous')."</a>&nbsp;&nbsp;&nbsp; \n";
    }
    ...
    for ($page = $page_inf; $page <= $page_sup; $page++) {
      if ($page == $actual_page) {
           $nav_bar .= " <span class=\"SearchHighlight\">$page</span> \n";
           $pages_bar .= " <span class=\"SearchHighlight\">$page</span> \n";
      }
      else {
          $nav_bar .= " <a href=\"".$url_bar.(($page-1)*$limite)."\" >$page</a> \n";
          $pages_bar .= " <a href=\"".$url_bar.(($page-1)*$limite)."\" >$page</a> \n";
      }
    }

    if ($more_results == 1) {
        $next_link = $url_bar.($lim_start+$limite);
        $nav_bar .= " &nbsp;&nbsp;&nbsp;<a href=\"$next_link\" >".phpdigMsg('next')."&gt;&gt;</a>\n";
    }

    $mtime = explode(' ',microtime());
    $search_time = sprintf('%01.2f',$mtime[0]+$mtime[1]-$start_time);
    $result_message = stripslashes(ucfirst(phpdigMsg('results'))." $n_start-$n_end, $num_tot ".phpdigMsg('total').", ".phpdigMsg('on')." \"".htmlspecialchars($query_string,ENT_QUOTES)."\"");
    ...
    if ((strlen(trim($leven_final)) > 0) && ($leven_sum > 0)) {
        ...
        $result_message .= ". " . phpdigMsg('alt_try') ." <a class=\"phpdigMessage\" href=\"".SEARCH_PAGE."?query_string=".urlencode($leven_query)."\"><i>".htmlspecialchars($leven_query,ENT_QUOTES)."</i></a>?";
    }
}
...
$title_message = stripslashes(ucfirst(phpdigMsg('s_results'))." ".phpdigMsg('on')." ".htmlspecialchars($query_string,ENT_QUOTES));
   ...
   $title_message = 'CreditCards.com Search '.PHPDIG_VERSION;
...
$powered_by_link .= "<a href=\"http://www.creditcards.com/\">".phpdigMsg('powered_by')."</a><br></font>";
...
<style>
.SearchHighlight {color:<?php print HIGHLIGHT_COLOR ?>;
...
<br />
<?php
phpdigMakeForm($query_string,$option,$limite,SEARCH_PAGE,$site,$path,'cc',$template_demo,$num_tot,$refine);
?>
...
<div align='center'>
<a href='http://www.creditcards.com/' target='_blank'><img src='phpdig_powered_2.png' width='88' height='28' border='0' alt='Powered by CreditCards.com' /></a> &nbsp;
</div>
</body>
</html>
<?php
...
$text = str_replace("&lt;span class=&quot;SearchHighlight&quot;&gt;","<span class=\"SearchHighlight\">",$text);
return $text;
}
