...
function validateMassAction(frm)
{
	if (frm.massaction.value != "")
	{
		...
		} else if (frm.massaction.value == "overrideTrans")
		{
			if (confirm("Override transactions?"))
			{
				return true;
			}
			
			return false;
		} else if (frm.massaction.value == "appendTrans")
		{
			if (confirm("Append transactions?"))
			{
				return true;
			}
...
function appendTrans(ID)
{
	if(confirm("Append transaction?"))
  	{
  		document.location.href = "index.php?md=Affiliate_Merchants_Views_UploadErrorManager&transid="+ID+"&action=appendTrans";
  	}
}

function rerateTrans(ID)
{
	if(confirm("Recalculate rate?"))
  	{
  		document.location.href = "index.php?md=Affiliate_Merchants_Views_UploadErrorManager&transid="+ID+"&action=rerateTrans";
  	}
}

function resubmitTrans(ID)
{
 	document.location.href = "index.php?md=Affiliate_Merchants_Views_UploadErrorManager&transid="+ID+"&action=resubmit";
}

function overrideTrans(ID)
{
	if(confirm("Override transaction?"))
  	{
  		document.location.href = "index.php?md=Affiliate_Merchants_Views_UploadErrorManager&transid="+ID+"&action=overrideTrans";
  	}
}
...
function deleteTrans(ID)
{
  ...
  {
  	document.location.href = "index.php?md=Affiliate_Merchants_Views_UploadErrorManager&transid="+ID+"&action=delete";
  }
}

function updateError(ID)
{
	var wnd = window.open("index_popup.php?md=Affiliate_Merchants_Views_UploadErrorManager&id="+ID+"&action=edit","Error_Adjustment","scrollbars=1, top=100, left=100, width=500, height=500, status=0");
    wnd.focus(); 
}
...
.redAlert {
	...
	<table border="0" cellpadding="0" cellspacing="0">
		<tr>
				    ...
				    <tr>
				    	<td style="padding: 5px;" valign="top">
					        ...
					        <option value=100000000 <? print ($_REQUEST['numrows']==100000000 ? "selected" : ""); ?>><?=L_G_ALL?></option>
					      	...
					      	&nbsp;&nbsp;&nbsp;&nbsp;<? $this->a_this->printErrorCodes(); ?>
					      	
							<br />
							<br />
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" class="formbutton" value="Apply Filter"> <input type="button" onclick="javascript:location.href='index.php?md=Affiliate_Merchants_Views_UploadErrorManager'" class="formbutton" value="Remove Filter">
						</td>
						...
						<td style="padding: 5px;" valign="top">
							...
							<!-- form closes in upload_error_list.tpl.php file -->
			...
			<td valign="top">
				 <table class="listing" border="0" cellspacing="0">
					<? QUnit_Templates::printFilter(10, "Error Codes Key"); ?>
					<tr>
					  <td style="padding: 5px;">
					  		<span style="color: #<?=$this->a_this->_getErrorColor('101'); ?>;">101</span>: TRANSID Field is Empty
							<br />
							<span style="color: #<?=$this->a_this->_getErrorColor('102'); ?>;">102</span>: Duplicate Transaction
							<br />
							<span style="color: #<?=$this->a_this->_getErrorColor('103'); ?>;">103</span>: No Revenue
							<br />
							<span style="color: #<?=$this->a_this->_getErrorColor('104'); ?>;">104</span>: No Corresponding Trans ID / User Click 
							<br />
							<span style="color: #<?=$this->a_this->_getErrorColor('105'); ?>;">105</span>: Missing Required Fields
							<br />
							<span style="color: #<?=$this->a_this->_getErrorColor('106'); ?>;">106</span>: Rate Discrepancy on Sync
							<br />
							<span style="color: #<?=$this->a_this->_getErrorColor('108'); ?>;">108</span>: Negative Revenue
							<br />
							<span style="color: #<?=$this->a_this->_getErrorColor('107'); ?>;">107</span>: Insert Date Error
                            <br />
                            <span style="color: #<?=$this->a_this->_getErrorColor('109'); ?>;">109</span>: Process Date Error
                            <br />
                            <span style="color: #<?=$this->a_this->_getErrorColor('110'); ?>;">110</span>: Event Date Error
                            <br />
                            <span style="color: #<?=$this->a_this->_getErrorColor('111'); ?>;">111</span>: Estimated Date Error
					  </td>
					</tr>
				 </table>
			</td>
		        ...
		        <table width="100%"  height="18" cellspacing="0" cellpadding="0">
			        <tr>
			            <td class=listViewLine>
			                <? $this->a_this->printAvailableViews('Affiliate_Merchants_Views_UploadErrorManager'); ?>
			            </td>
			        </tr>
		        </table>
