        ...
        <tr>
          ...
          <td class=dir_form valign=top><?=L_G_STATUS;?></td>
          <td valign=top>
            <select name=status>
        ...
        <? if($GLOBALS['Auth']->getSetting('Aff_join_campaign') == '1') { ?>
        <tr>
          <td class=dir_form valign=top><?=L_G_SIGNUP_BONUS?></td>
          <td valign=top>
          <? if($this->a_Auth->getSetting('Aff_currency_left_position') == '1')
                print '&nbsp;'.$this->a_Auth->getSetting('Aff_system_currency').'&nbsp;';
          ?>
          <input type=text name=signup_bonus size=5 value="<?=$_POST['signup_bonus']?>">
          <? if($this->a_Auth->getSetting('Aff_currency_left_position') != '1')
                print '&nbsp;'.$this->a_Auth->getSetting('Aff_system_currency').'&nbsp;';
          ?>
          </td>
