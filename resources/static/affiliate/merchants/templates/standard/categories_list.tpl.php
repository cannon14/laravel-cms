<? 
include('categories_filter.tpl.php');
...
<? } ?>
   ...
   <table class=listingClosed border=0 cellspacing=0 cellpadding=0>
    <? QUnit_Templates::printFilter(10, "View Categories"); ?>
    <tr>
        <td class=actionheader align=left>
          <? if($this->a_action_permission['view']) { ?>
            <b><a class=mainlink href="javascript:createSite('<?=$_REQUEST['type']?>')">Create New Category </a> | <a class=mainlink href="javascript:FilterForm.action.value='exportcsv'; FilterForm.submit();"><?=L_G_EXPORTTOCSV?></a></b>
          <? } ?>
        </td>
    </tr>    
    <tr>
        ...
        <tr>
            <td class=listViewLine>
                <? $this->a_this->printAvailableViews('Affiliate_Merchants_Views_CategoryManager'); ?>
            </td>
        </tr>
        </table>
...
<input type=hidden id=action name=type value='<?=$_REQUEST['type']?>'>    
</form>
