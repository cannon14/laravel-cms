...
function addExpense()
{
	var wnd = window.open("index_popup.php?md=Affiliate_Merchants_Views_ExpensesUploadErrorsManager&action=create&type=all","Expense","scrollbars=1, top=100, left=100, width=600, height=500, status=0");
    wnd.focus(); 
}
function editExpense(ID)
{
	var wnd = window.open("index_popup.php?md=Affiliate_Merchants_Views_ExpensesUploadErrorsManager&action=edit&eid="+ID,"Expense","scrollbars=1, top=100, left=100, width=600, height=500, status=0");
    wnd.focus(); 
}

function resubmitExpense(ID)
{
	var wnd = window.open("index_popup.php?md=Affiliate_Merchants_Views_ExpensesUploadErrorsManager&action=resubmit&eid="+ID,"Expense","scrollbars=1, top=100, left=100, width=600, height=500, status=0");
    wnd.focus(); 
}
...
function deleteExpense(ID)
{
  ...
  if(confirm("<?=L_G_CONFIRMDELETEEXPENSE?> - "+ID))
   	document.location.href = "index.php?md=Affiliate_Merchants_Views_ExpensesUploadErrorsManager&type=all&eid="+ID+"&action=delete";
...
<form name=FilterForm id=FilterForm action=index.php method=get>
    ...
    <? QUnit_Templates::printFilter(10, "Expense Upload Errors Filter"); ?>
...
<?    while($data=$this->a_list_users->getNextRecord()) { ?>
        <option value="<?=$data['userid']?>" <?=($_REQUEST['exp_affiliate_id'] == $data['userid'] ? 'selected' : '')?>><?=$data['name']!='' ? $data['name'] : $data['username']?></option>
<?    } ?>      
      </select>
      </td>
      
      <td rowspan="6" nowrap style="border-left: 1px solid #000;">
	    ...
	    echo "<br>";
		echo "<INPUT TYPE=RADIO NAME='date' VALUE='e.errordate' " . ($_REQUEST['date'] == 'e.errordate' ? 'CHECKED' : '' ) . "> Error Date";
		echo "<br>";
